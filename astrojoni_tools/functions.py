...
def latitude_axes(name: Path) -> np.ndarray:
    ...
    return (header["CRVAL2"] - header["CRPIX2"] * header["CDELT2"]) + (
        np.arange(n) + 1
    ) * header["CDELT2"]
...
def longitude_axes(name: Path) -> np.ndarray:
    ...
    return (header["CRVAL1"] - header["CRPIX1"] * header["CDELT1"]) + (
        np.arange(n) + 1
    ) * header["CDELT1"]


def world_to_pixel(
    fitsfile: Path, longitude: float, latitude: float, velocity: Optional[float] = 0.0
) -> np.ndarray:
...
def pixel_to_world(fitsfile: Path, x: float, y: float, ch: Optional[float] = 0.0):
...
def calculate_average_value_of_map(
    fitsfile: Path, pixel_array: List
) -> Tuple[float, List]:
...
def moment_0(
    filename: Path,
    velocity_start: float = None,
    velocity_end: float = None,
    ...
    print(
        "velocity-range: "
        + str(velocity[lower_channel])
        + " - "
        + str(velocity[upper_channel])
    )
    ...
    newname = (
        filename_base
        + f"_mom-0_{np.around(velocity[lower_channel],decimals=1):05.1f}_to_{np.around(velocity[upper_channel],decimals=1):05.1f}km-s"
        + suffix
        + ".fits"
    )
...
def moment_1(
    ...
    newname = (
        filename_base
        + "_mom-1_"
        + str(np.around(velocity[lower_channel], decimals=1))
        + "_to_"
        + str(np.around(velocity[upper_channel], decimals=1))
        + "km-s"
        + suffix
        + ".fits"
    )
...
def pixel_circle_calculation(
    ...
    central_px = [
        int(np.round(central_px[0], decimals=0)) - 1,
        int(np.round(central_px[1], decimals=0)) - 1,
    ]
    if r != "single":
        circle_size_px = 2 * r / 3600.0 / delta
        circle_size_px = int(round(circle_size_px))
        px_start = [
            central_px[0] - circle_size_px / 2,
            central_px[1] - circle_size_px / 2,
        ]
        px_end = [
            central_px[0] + circle_size_px / 2,
            central_px[1] + circle_size_px / 2,
        ...
        px_start = [
            int(np.round(px_start[0], decimals=0)),
            int(np.round(px_start[1], decimals=0)),
        ]
        px_end = [
            int(np.round(px_end[0], decimals=0)),
            int(np.round(px_end[1], decimals=0)),
        ]
        for i_x in trange(px_start[0] - 1, px_end[0] + 1):
            for i_y in range(px_start[1] - 1, px_end[1] + 1):
                if (
                    np.sqrt((i_x - central_px[0]) ** 2 + (i_y - central_px[1]) ** 2)
                    < circle_size_px / 2.0
                ):
...
def pixel_circle_calculation_px(
    fitsfile: Path, x: Union[np.ndarray, float], y: Union[np.ndarray, float], r: float
) -> Tuple[List, Tuple]:
    """Extract both a list of pixels [(y0,x0),(y1,x1),...] and a tuple ((y0,y1,...),(x0,x1,...))
...
def pixel_box_calculation(
    fitsfile: Path, longitude: float, latitude: float, a: float, b: float
):
    ...
    if a != "single":
        box_size_px_a = a / 3600.0 / delta
        box_size_px_a = int(round(box_size_px_a))
        box_size_px_b = b / 3600.0 / delta
        box_size_px_b = int(round(box_size_px_b))
        px_start = [
            central_px[0] - box_size_px_a / 2,
            central_px[1] - box_size_px_b / 2,
        ]
...
def pixel_annulus_calculation(fitsfile, longitude, latitude, r_in, r_out):
    ...
    w = WCS(fitsfile)
    delta = abs(header["CDELT1"])  # in degree
    circle_in_px = 2 * r_in / 3600.0 / delta
    circle_in_px = int(round(circle_in_px))
    circle_out_px = 2 * r_out / 3600.0 / delta
    ...
    pixel_array = []
    for i_x in trange(
        int(np.round(px_start[0], decimals=0) - 1),
        int(np.round(px_end[0], decimals=0) + 1),
    ):
        for i_y in range(
            int(np.round(px_start[1], decimals=0) - 1),
            int(np.round(px_end[1], decimals=0) + 1),
        ):
            if (
                np.sqrt((i_x - central_px[0]) ** 2 + (i_y - central_px[1]) ** 2)
                > circle_in_px / 2.0
...
def pixel_ellipse_annulus_calculation(center_x, center_y, x_in, x_out, y_in, y_out):
    ...
    """
    central_px = [
        int(np.round(center_x, decimals=0)),
        int(np.round(center_y, decimals=0)),
    ]
    ...
    if x_in == 0 and y_in == 0:
        for i_x in trange(
            ...
            for i_y in range(
                ...
                if (
                    ((i_x - center_x) / float(x_out)) ** 2
                    + ((i_y - center_y) / float(y_out)) ** 2
                ) < 1:
                    pixel_coords.append((i_y, i_x))
    else:
        for i_x in trange(
            ...
            for i_y in range(
                ...
                if (
                    ...
                    ((i_x - center_x) / float(x_in)) ** 2
                    + ((i_y - center_y) / float(y_in)) ** 2
                ) > 1:
                    pixel_coords.append((i_y, i_x))
    indices_np = tuple(zip(*pixel_coords))
    return pixel_coords, indices_np
...
def make_subcube(
    ...
    if latitudes is not None:
        ...
        lat_range_idx = sorted(
            [find_nearest(b, lat_range[0]), find_nearest(b, lat_range[1])]
        )
    ...
    if longitudes is not None:
        ...
        lon_range_idx = sorted(
            [find_nearest(l, lon_range[0]), find_nearest(l, lon_range[1])]
        )
    ...
    if cube.ndim == 3:
        if velo_range is not None:
            ...
            vel_range_idx = sorted(
                [find_nearest(v, vel_range[0]), find_nearest(v, vel_range[1])]
            )
    ...
    if cube.ndim == 3:
        sub_cube = cube[
            vel_range_idx[0] : vel_range_idx[1],
            lat_range_idx[0] : lat_range_idx[1],
            lon_range_idx[0] : lon_range_idx[1],
        ]
    elif cube.ndim == 2:
        sub_cube = cube[
            lat_range_idx[0] : lat_range_idx[1], lon_range_idx[0] : lon_range_idx[1]
        ]
    ...
    for vel in trange(data.shape[0]):
        for lon in range(data.shape[2]):
            if mode == "avg":
                ...
                norm_weight = np.absolute(weight[vel, :, lon]) / np.nansum(
...
def jansky_to_kelvin(frequency, theta_1, theta_2):  # in units of (GHz,arcsec,arcsec)
    c = const.c
    k = const.k_B
    theta_1 = theta_1 * 2 * np.pi / 360.0 / 3600.0
    theta_2 = theta_2 * 2 * np.pi / 360.0 / 3600.0
    S = (
        2
        * k
        * (frequency * 10**9) ** 2
...
def find_common_beam(filenames):
    ...
    common_beam = radio_beam.commonbeam.common_2beams(
        radio_beam.Beams(beams=[cube1.beam, cube2.beam])
    )
    return common_beam
...
def spatial_smooth(
    ...
    **kwargs
    ...
    filename_base, file_extension = os.path.splitext(filename_wext)
    newname = (
        filename_base + f"_smooth_{meanbeam.value}_{meanbeam.unit}" + suffix + ".fits"
    )
    pathname = os.path.join(path_to_output, newname)
    ...
    if datatype == "large":
        ...
        with tqdm(total=cube.shape[0]) as pbar:
            for index in range(cube.shape[0]):
                smooth_slice = cube[index].convolve_to(
                    beam, **kwargs
                )  # still reads in copy of cube to store convolution; should be fixed
                outfh[0].data[index] = smooth_slice.array
                outfh.flush()  # write the data to disk
                pbar.update(1)
        outfh[0].header.update(beam.to_header_keywords())
        outfh[0].header.update(
            {
                "BEAM": f"Beam: BMAJ={beam.major.value:.1f} arcsec BMIN={beam.minor.value:.1f} arcsec BPA={beam.pa.value:.1f} deg"
            }
        )
        outfh.flush()
        print("\n\033[92mSAVED FILE:\033[0m '{}'".format(newname))
...
def spectral_smooth(
    ...
    **kwargs
    ...
    filename_base, file_extension = os.path.splitext(filename_wext)
    newname = (
        filename_base + f"_spectral_smooth_{target.value:.1f}kms" + suffix + ".fits"
    )
    pathname = os.path.join(path_to_output, newname)
    ...
    if datatype == "large":
        ...
        outfh[0].header.comments[
            "CDELT3"
        ] = "Channel width but spectral res. is " + target.to_string(
            unit=u.m / u.s, precision=1
        )
        outfh.flush()
        print("\n\033[92mSAVED FILE:\033[0m '{}'".format(newname))
    else:
        ...
        if spectrum.shape[1] == 2:
            y = np.nanmean(
                np.pad(
                    ...
                    (0, ((2 - spectrum[:, 1].size % 2) % 2)),
                    mode="edge",
                ).reshape(-1, 2),
                axis=1,
            )  # pad with edge_values at the end if odd number of channels
            x = np.nanmean(
                np.pad(
                    spectrum[:, 0].astype(float),
                    (0, ((2 - spectrum[:, 0].size % 2) % 2)),
                    mode="edge",
                ).reshape(-1, 2),
                axis=1,
            )
            ...
            raise ValueError(
                "Array needs to be of shape (N, 2) but instead has shape {}".format(
                    spectrum.shape
                )
            )
    else:
...
def smooth_2(spectrum):
    new_spectrum = np.repeat(
        np.nanmean(
            np.pad(
                spectrum.astype(float), (0, ((2 - spectrum.size % 2) % 2)), mode="edge"
            ).reshape(-1, 2),
            axis=1,
        ),
        repeats=2,
...
def calculate_pixelArray_along_filament(
    filename, filamentdata, halfwidth_pc, distance_of_source
):
    ...
    header = fits.getheader(filename)
    pixel_scale = (
        math.tan(np.abs(header["CDELT1"]) * math.pi / 180) * distance_of_source
    )  # pc per px
    ...
    for n in trange(1, len(filament_coords) - 1):
        ...
        if header["NAXIS"] == 2:
            ...
            raise Exception(
                "FITS file must be 2D or 3D. Instead, file has shape {}".format(
                    intensity.shape
                )
            )
        ...
        if -float(b) / a > 0:
            ...
            for ii in range(y0 - npix, y0 + 1):
                for jj in range(x0 - npix, x0 + 1):
                    distance[ii, jj] = (
                        (jj - x0) ** 2.0 + (ii - y0) ** 2.0
                    ) ** 0.5  # distance between point (i,j) and filament
                    if distance[ii, jj] < npix - 1:
                        dist_normal = (np.fabs(a * jj + b * ii - const)) / (
                            a**2 + b**2
                        ) ** 0.5  # distance between point (i,j) and the normal
                        # take the point if it is in the vicinity of the normal (distance < 2 pix)
                        if dist_normal < 1:
                            line_perp[ii, jj] = distance[
                                ii, jj
                            ]  # storing the nearby points
                            line_perpList.extend((ii, jj, distance[ii, jj]))

            for ii in range(y0, y0 + npix):
                for jj in range(x0, x0 + npix):
                    distance2[ii, jj] = ((jj - x0) ** 2.0 + (ii - y0) ** 2.0) ** 0.5
                    if distance2[ii, jj] < npix - 1:
                        dist_normal2 = (np.fabs(a * jj + b * ii - const)) / (
                            np.sum(normal * normal)
                        ) ** 0.5
        ...
        # Loop 2_ if the slope is positive
            ...
            for ii in range(y0, y0 + npix):
                for jj in range(x0 - npix, x0 + 1):
                    distance[ii, jj] = ((jj - x0) ** 2.0 + (ii - y0) ** 2.0) ** 0.5
                    if distance[ii, jj] < npix - 1:
                        dist_normal = (np.fabs(a * jj + b * ii - const)) / (
                            np.sum(normal * normal)
                        ) ** 0.5
    ...
    print(
        "Filament positions selected over "
        + str(2 * halfwidth_pc)
        + " pc slices perpendicular to filament"
    )
...
def estimate_cont_noise(data, inner_pct, outer_pct):
    ...
    y_size = data.shape[0]
    x_size = data.shape[1]
    central_px_x = x_size / 2.0
    central_px_y = y_size / 2.0
    px_array, indices = pixel_ellipse_annulus_calculation(
        central_px_x,
        central_px_y,
        inner_pct * x_size / 2.0,
        outer_pct * x_size / 2.0,
        inner_pct * y_size / 2.0,
        outer_pct * y_size / 2.0,
    )
